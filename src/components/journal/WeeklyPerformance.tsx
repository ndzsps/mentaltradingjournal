...
export const WeeklyPerformance = () => {
  ...
  if (isLoading) {
    return (
      <div className="grid grid-rows-5 gap-4 h-[calc(100vh-12rem)]">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="flex items-center px-2">
            <Card className="w-full h-24 p-4 space-y-2 bg-primary/5">
  ...
  return (
    <div className="flex flex-col gap-4 h-[calc(100vh-12rem)]">
      {weeklyStats?.map((week) => (
        <div key={week.weekNumber} className="px-2">
          <Card
            className="w-full h-24 p-4 bg-card/30 backdrop-blur-xl border-primary/10 hover:border-primary/20 transition-colors flex flex-col justify-center"
          >
