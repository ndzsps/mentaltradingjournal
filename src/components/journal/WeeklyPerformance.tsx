...
export const WeeklyPerformance = () => {
  ...
  if (isLoading) {
    return (
      <div className="grid grid-rows-5 gap-[0.35rem] h-[calc(100vh-12rem)] pt-[3.55rem]">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="flex items-center px-2">
            <Card className="p-4 space-y-2 bg-primary/5 w-full h-[5.95rem]">
    ...
    <div className="grid grid-rows-5 gap-[0.35rem] h-[calc(100vh-12rem)] pt-[3.55rem]">
      {weeklyStats?.map((week) => (
        <div key={week.weekNumber} className="flex items-center px-2">
          <Card
            className="p-4 bg-card/30 backdrop-blur-xl border-primary/10 hover:border-primary/20 transition-colors w-full h-[5.95rem] flex flex-col justify-center"
          >
